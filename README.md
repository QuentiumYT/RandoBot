![RandoBot](./images/banner.png)

<h1 align="center" style="font-size: 50px;">Welcome to RandoBot project 👋</h1>

[![Donate](https://quentium.fr/+img/github/donate.svg)](https://www.paypal.me/QuentiumYT/1)&nbsp;
[![Contributions](https://quentium.fr/+img/github/contributions.svg)](https://github.com/QuentiumYT/RandoBot/pulls)&nbsp;
[![Tested on Python 3.10](https://quentium.fr/+img/github/python310.svg)](https://www.python.org/downloads)&nbsp;
[![License](https://quentium.fr/+img/github/mit.svg)](https://github.com/QuentiumYT/RandoBot/blob/master/LICENSE)&nbsp;
[![Size](https://img.shields.io/github/repo-size/QuentiumYT/RandoBot?label=Repo%20Size&color=4391BD&logo=data:image/png;base64,)](../../)&nbsp;

## ✨ Introduction

RandoBot is a bot with features to easily organize hikes. Basically, it allows you to propose a hike in an announcement channel on a specific day.

Every command or set of commands are located in the `cogs` folder. If you need a specific addition, create an issue with your need or a pull request and I will add it to the bot.

⚠️ Translations are not done yet, every string is in french :p

## 🚀 Features

-   `/rando` ➤ Organize a hike by filling a form & selects
-   `/participants` ➤ Show a list of people that are sure and unsure about joining the hike
-   `/archive` ➤ Move the hike channel in a category and save all pictures locally
-   Cogs manipulation commands `load`, `unload`, `reload` for admin only

## 📚 Documentation

Every commands are self explanatory, the help given by the descriptions should help you :)

[![Join RandoBot Support Server](https://discordapp.com/api/guilds/380373195473027074/embed.png?style=banner4)](https://discord.gg/5sehgXx)

## 🤝 Contributing

Pull requests are welcome and please report any issues you find in [issues](https://github.com/QuentiumYT/RandoBot/issues). For major changes, please open an issue first to discuss what you would like to change or create a new branch.

Please make sure to test your suggestions before committing.

## 👤 Author

**Quentin L.**

-   Website: [quentium.fr](https://quentium.fr/)
-   Twitter: [@QuentiumYT](https://twitter.com/QuentiumYT)
-   Contact: [Mail](mailto:pro@quentium.fr?subject=[Bot]%20Contact%20for%20...)

Please ✰ this repository if this project helped you!

## 📖 License

[MIT](https://choosealicense.com/licenses/mit/)

---

_Made with_ ❤ _by QuentiumYT_
